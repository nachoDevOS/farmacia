...
@section('content')
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form"
            ...
            {{-- action="@if (!is_null($dataTypeContent->getKey())){{ route('update.users', $dataTypeContent->getKey()) }}@else{{ route('store.users') }}@endif" --}} method="POST" enctype="multipart/form-data" autocomplete="off">
                ...
                <div class="col-md-8">
                    <div class="panel panel-bordered">
                        ...
                        <div class="panel-body">
                            ...
                            @if (!$dataTypeContent->getKey())
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <select name="person_id" id="select-person_id" required
                                            class="form-control"></select>
                                        <span class="input-group-btn">
                                            <button class="btn btn-primary" title="Nueva persona"
                            ...
                            @endif
                            @php
                                $branches = App\Models\Branch::where('status', 1)->where('deleted_at', null)->get();
                            @endphp
                            <div class="form-group">
                                <label for="branch_id">Sucursal</label>
                                <select name="branch_id" id="branch_id" required class="form-control select2">
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" class="form-control" id="email" name="email"
                                    {{ $dataTypeContent->getKey() ? 'disabled' : '' }}
                                    ...
                                    <select name="role_id" id="role_id" class="form-control select2" required>
                                        ...
                                        @foreach ($role as $item)
                                            <option value="{{ $item->id }}"
                                                @if ($dataTypeContent) {{ $dataTypeContent->role_id == $item->id ? 'selected' : '' }} @endif>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            @if ($dataTypeContent->getKey())
                                <div class="form-group">
                                    ...
                                    <input type="checkbox" name="status" class="toggleswitch"
                                        @if (isset($dataTypeContent->id)) {{ $dataTypeContent->status == 1 ? 'checked' : '' }} 
                                        @else 
                                            checked @endif
